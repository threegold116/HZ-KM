...
# 给定输入进行分类处理
def test(config_path,model_path,translate_model_path,input_file_path,output_path):
    # 配置参数
    config = parse_option(config_path)
    ...
    for root, dirs, files in os.walk(input_file_path):
        ...
        for name in files:
            ...
            try:
                # 对于 Image_caption 直接跳过判断
                if task_name == "Image_caption" and count_caption > reduce_caption_num:
                    # raise RuntimeError("放弃部分任务二")
                    read_question_dict[answer_key].append("图像显示的是海上场景，有一个停车场，机场中间有3驾飞机停靠。")

                # 只有 QA和 caption 经过LLM
                # if task_name == "QA" or task_name == "Image_caption":
                # 只有 QA 经过LLM
                if task_name == "QA" or (task_name == "Image_caption" and count_caption <= reduce_caption_num):
                    for input_prompt in promots:

                        # 模型进行推理
                        outputs = eval_model(config,input_prompt,image_path,tokenizer, model, vision_processor)                        
                        # 生成总结果
                        read_question_dict[answer_key].append(outputs)
                        # print(read_question_dict)
                
                # read_question_dict[answer_key] 结果翻译 map+机器翻译
                # 只有 QA 和改变了的 change 进行翻译
                if task_name == "QA" or (task_name == "Change_caption" and change_flag) or (task_name == "Image_caption" and count_caption <= reduce_caption_num):
...
def eval_model(config,input_prompt,image_path,tokenizer, model, vision_processor):

    # 初始化prompt模板与角色设置
    conv = conv_templates["remote_sense"].copy()
    dtype = type_dict[config.dtype]
    ...
    if image_path is not None:
        ...
        if config.rgb_vision.arch.startswith("vit"):
            ...
            image_tensor = vision_processor(image).to(dtype).to(device="cuda").unsqueeze(0)
    else:
        image = None
        image_tensor = None
    ...
    outputs = outputs.split("</s>")[0].strip() # remove </s>
    return outputs
...
if __name__ == "__main__":
    ...
    # llava-1.5预训练权重
    config_path = "Config/multi_modal_eval.yaml"
    # 三阶段微调后的最终权重
    model_path = "checkpoint/Stage3/FINAL.pt"
    # 翻译模型
    translate_model_path = "checkpoint/translate_entozh"
    # test_data_path = "input_path"
    # output_path = "output_path"
    test(config_path,model_path,translate_model_path,test_data_path,output_path)
